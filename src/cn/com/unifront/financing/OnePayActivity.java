...
public class OnePayActivity extends Activity implements OnClickListener {
	...
	private void getview() {
		et_onepay_entermoney = (EditText) this
				.findViewById(R.id.et_onepay_entermoney);
	...
	public void onClick(View v) {
		...
		case R.id.bt_onepay_ture:// 选择确定添加按钮事件处理	
			...
			if("0.0".equals(moneyCheck)==false){
				float money = Float.parseFloat("-"
						+ et_onepay_entermoney.getText().toString().trim());
				...
				operate.insertPayIncomeDate(1,money, time, account, type, remark);
				
				Toast.makeText(OnePayActivity.this, "记账成功！"+account, Toast.LENGTH_SHORT)
						.show();
				finish();
			}else{
				Toast.makeText(OnePayActivity.this, "请输入金额！！", Toast.LENGTH_SHORT)
				.show();
			}				
